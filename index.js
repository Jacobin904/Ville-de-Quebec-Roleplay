...
const CONFIG = {
    ...
    // ⚠️ REMPLACE CES ID PAR LES VRAIS RÔLES DE TON SERVEUR
    departments: {
        spvq: {
...
// ==========================================
// 2. GESTIONNAIRE DE BASE DE DONNÉES AVANCÉ
// ==========================================
class DatabaseManager {
    ...
    scheduleSave() {
        ...
        this.saveTimeout = setTimeout(() => this.save(), 3000); // Délai de 3 secondes pour regrouper les écritures
    }
...
// ==========================================
// 3. SYSTÈME DE JOURNALISATION (LOGGER)
// ==========================================
class Logger {
    ...
    static async discord(title, description, color = CONFIG.colors.primary, fields = [], thumbnail = null) {
        ...
        try {
            ...
            this.error(`Échec de l'envoi du journal Discord : ${err.message}`);
        }
    }
}

// ==========================================
// 4. REGISTRE DE COMMANDES UNIFIÉ (SLASH & PREFIX)
// ==========================================
class CommandRegistry {
    ...
    async executePrefix(message, args) {
        const command = this.commands.get(args[0].slice(1).toLowerCase());
        ...
        const mockInteraction = {
            ...
            options: {
                ...
                getString: (name) => args.slice(2).join(' '),
    ...
    async _execute(command, context, message) {
        try {
            ...
            if (message && command.permissions) {
                ...
                if (!hasPermission) {
                    return context.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription('Vous ne possédez pas les permissions requises pour exécuter cette commande.').setColor(CONFIG.colors.danger)] });
                }
            }
            ...
            const errorMsg = { embeds: [new EmbedBuilder().setTitle('Erreur Interne').setDescription('Une erreur inattendue s\'est produite lors du traitement de votre demande.').setColor(CONFIG.colors.danger)] };
...
function setupCommands() {
    ...
    registry.register({
        ...
        execute: async (ctx, client) => {
            await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Latence').setDescription(`Latence de l\'API : **${client.ws.ping} ms**`).setColor(CONFIG.colors.primary)] });
        }
    });

    // --- COMMANDE : BAN (Slash & Prefix) ---
    registry.register({
        name: 'ban',
        ...
        execute: async (ctx, client, database) => {
            ...
            if (target.id === ctx.user.id) return ctx.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription('Vous ne pouvez pas vous bannir vous-même.').setColor(CONFIG.colors.danger)] });
            if (ctx.member.roles.highest.position <= target.roles.highest.position) return ctx.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription('Vous ne pouvez pas bannir un membre ayant un rôle supérieur ou égal au vôtre.').setColor(CONFIG.colors.darning)] });

            try {
                await target.ban({ reason: `Banni par ${ctx.user.tag} : ${reason}` });
                await Logger.discord('Membre Banni', `**${target.user.tag}** a été banni du serveur.`, CONFIG.colors.danger, [
                    { name: 'Modérateur', value: `${ctx.user.tag} (${ctx.user.id})`, inline: true },
                    { name: 'Motif', value: reason, inline: true }
                ], target.user.displayAvatarURL());
                
                await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Succès').setDescription(`${target.user.tag} a été banni avec succès.`).setColor(CONFIG.colors.success)] });
            } catch (error) {
                await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription(`Impossible de bannir ce membre : ${error.message}`).setColor(CONFIG.colors.danger)] });
            }
        }
    });

    // --- COMMANDE : PROMOTION DÉPARTEMENTALE (Exclusif Directeurs) ---
    registry.register({
        name: 'promote',
        slashData: new SlashCommandBuilder().setName('promote').setDescription('Promouvoir un membre dans votre département (Réservé aux directeurs).').addUserOption(o => o.setName('utilisateur').setDescription('Le membre à promouvoir').setRequired(true)).addStringOption(o => o.setName('département').setDescription('Le département concerné').setRequired(true).addChoices({ name: 'SPVQ', value: 'spvq' }, { name: 'SPCIQ', value: 'spciq' }, { name: 'SQ', value: 'sq' })).addStringOption(o => o.setName('grade').setDescription('Le nouveau grade').setRequired(true)),
        ...
        execute: async (ctx, client, database) => {
            ...
            if (!dept) return ctx.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription('Département invalide. Choisissez parmi : spvq, spciq, sq.').setColor(CONFIG.colors.danger)] });

            // 1. Vérifier si l'exécutant est le directeur de ce département
            if (!ctx.member.roles.cache.has(dept.roles.director)) {
                return ctx.reply({ embeds: [new EmbedBuilder().setTitle('Accès Refusé').setDescription(`Seul le directeur du ${dept.name} est autorisé à effectuer des promotions.`).setColor(CONFIG.colors.danger)] });
            }

            // 2. Vérifier si le nouveau grade existe dans ce département
            const targetRoleId = dept.roles[newGradeKey];
            if (!targetRoleId) {
                const validGrades = Object.keys(dept.roles).join(', ');
                return ctx.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription(`Grade invalide. Grades disponibles pour ce département : ${validGrades}.`).setColor(CONFIG.colors.danger)] });
            }

            // 3. Vérifier si la cible est déjà dans ce département (optionnel, mais recommandé)
            const isInDept = Object.values(dept.roles).some(roleId => member.roles.cache.has(roleId));
            
            try {
                // Retirer les anciens rôles de département (pour éviter les accumulations)
                ...
                const roleName = new Object({ [newGradeKey]: newGradeKey })[newGradeKey]; // Mapping simple pour le nom
                
                await Logger.discord('Promotion Départementale', `**${target.user.tag}** a été promu au grade de **${newGradeKey}** dans le **${dept.name}**.`, CONFIG.colors.success, [
                    { name: 'Promu par', value: `${ctx.user.tag}`, inline: true }
                ], target.user.displayAvatarURL());

                await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Promotion Réussie').setDescription(`${target.user.tag} a été promu avec succès au grade de **${newGradeKey}** au sein du ${dept.name}.`).setColor(CONFIG.colors.success)] });
            } catch (error) {
                await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription(`Échec de la modification des rôles : ${error.message}`).setColor(CONFIG.colors.danger)] });
            }
        }
    });
    ...
    registry.register({
        name: 'warn',
        slashData: new SlashCommandBuilder().setName('warn').setDescription('Avertit un membre.').setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers).addUserOption(o => o.setName('membre').setDescription('Le membre à avertir').setRequired(true)).addStringOption(o => o.setName('raison').setDescription('Motif de l\'avertissement').setRequired(true)),
        ...
        execute: async (ctx, client, database) => {
            ...
            database.data.warnings.push({ user_id: target.id, moderator_id: ctx.user.id, reason, created_at: new Date().toISOString() });
            database.addWarning(target.id);
            
            await Logger.discord('Avertissement Émis', `**${ctx.user.tag}** a averti **${target.tag}**.`, CONFIG.colors.warning, [
                { name: 'Motif', value: reason, inline: false }
            ], target.displayAvatarURL());
            
            try {
                await target.send({ embeds: [new EmbedBuilder().setTitle('Avertissement Officiel').setDescription(`Vous avez reçu un avertissement sur **${ctx.guild.name}**.\n\n**Motif :** ${reason}`).setColor(CONFIG.colors.warning).setTimestamp()] });
            } catch (e) {}
            
            const updated = database.getUser(target.id);
            await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Succès').setDescription(`${target.tag} a été averti. (Total des avertissements : ${updated.warnings})`).setColor(CONFIG.colors.success)] });
        }
    });
    ...
    registry.register({
        name: 'clear',
        slashData: new SlashCommandBuilder().setName('clear').setDescription('Supprime des messages.').setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages).addIntegerOption(o => o.setName('nombre').setDescription('Nombre de messages à supprimer (max 100)').setRequired(true).setMinValue(1).setMaxValue(100)),
        permissions: [PermissionFlagsBits.ManageMessages],
        prefixHandler: true,
        execute: async (ctx, client, database) => {
            const amount = ctx.options.getInteger('nombre');
            await ctx.channel.bulkDelete(amount, true);
            await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Succès').setDescription(`${amount} messages ont été supprimés avec succès.`).setColor(CONFIG.colors.success)] });
            await Logger.discord('Messages Supprimés', `**${ctx.user.tag}** a supprimé **${amount}** messages dans le salon <#${ctx.channel.id}>.`, CONFIG.colors.warning, [], ctx.user.displayAvatarURL());
        }
    });
    ...
    registry.register({
        name: 'ticket',
        slashData: new SlashCommandBuilder().setName('ticket').setDescription('Ouvre un ticket de support.'),
        execute: async (ctx, client, database) => {
            // Note: Pour la version préfixée, on simplifie en créant directement le salon
            if (ctx.channel?.send) {
                ...
                await ticketChannel.send({ 
                    content: `${ctx.user}`, 
                    embeds: [new EmbedBuilder().setTitle('Nouveau Ticket').setDescription('Un membre de l\'équipe de direction va prendre en charge votre demande dans les plus brefs délais.').setColor(CONFIG.colors.primary).setTimestamp()] 
                });
                
                await ctx.reply({ embeds: [new EmbedBuilder().setTitle('Succès').setDescription(`Votre ticket a été créé : ${ticketChannel}`).setColor(CONFIG.colors.success)] });
            }
        }
    });
    ...
    registry.register({
        name: 'userinfo',
        slashData: new SlashCommandBuilder().setName('userinfo').setDescription('Affiche les informations détaillées d\'un utilisateur.').addUserOption(o => o.setName('membre').setDescription('Le membre concerné')),
        execute: async (ctx, client, database) => {
            ...
            await ctx.reply({
                embeds: [new EmbedBuilder().setTitle(`Informations : ${target.user.username}`).setColor(CONFIG.colors.primary).setThumbnail(target.user.displayAvatarURL())
                    .addFields(
    ...
    registry.register({
        name: 'scan',
        slashData: new SlashCommandBuilder().setName('scan').setDescription('Analyse complète du serveur et génération de 5 fichiers JSON.'),
        execute: async (ctx, client, database) => {
            ...
            for (let i = 0; i < 5; i++) {
                const metaData = { 
                    _meta: { file: i + 1, total_files: 5, server: guild.name, timestamp: new Date().toISOString(), item_count: buckets[i].items.length }, 
                    data: buckets[i].items 
                };
                files.push({ attachment: Buffer.from(JSON.stringify(metaData, null, 2), 'utf-8'), name: `analyse_partie_${i + 1}_sur_5_${safeName}_${timestamp}.json` });
            }
            
            await ctx.followUp({ 
                embeds: [new EmbedBuilder().setTitle('Analyse Terminée').setDescription('L\'analyse a généré **5 fichiers JSON** contenant l\'intégralité des données du serveur.').setColor(CONFIG.colors.success).setTimestamp()], 
                files: files, 
                ephemeral: true 
            });
        }
...
class VQCBot {
    ...
    setupEvents() {
        this.client.once('clientReady', async () => {
            ...
            await Logger.discord('Système Démarré', `Le bot est désormais opérationnel.\n\n**Identité :** ${this.client.user.tag}`, CONFIG.colors.success, [], this.client.user.displayAvatarURL());
        ...
        this.client.on('messageCreate', async (message) => {
            ...
            if (now - user.last_xp > CONFIG.limits.xpCooldown) {
                ...
                if (newXP >= xpNeeded) {
                    newXP -= xpNeeded;
                    newLevel++;
                    message.reply({ embeds: [new EmbedBuilder().setTitle('Niveau Supérieur').setDescription(`Félicitations ${message.author}, vous êtes passé au niveau **${newLevel}**.`).setColor(CONFIG.colors.success)] }).catch(() => {});
                }
            ...
            if (interaction.isChatInputCommand()) {
                ...
                if (interaction.customId === 'close_ticket') {
                    if (!interaction.channel.name.startsWith('ticket-')) {
                        return interaction.reply({ embeds: [new EmbedBuilder().setTitle('Erreur').setDescription('Cette action n\'est disponible que dans un salon de ticket.').setColor(CONFIG.colors.danger)], ephemeral: true });
                    }
                    db.closeTicket(interaction.channel.id);
                    await interaction.reply({ embeds: [new EmbedBuilder().setTitle('Fermeture en Cours').setDescription('Le ticket sera fermé dans 5 secondes.').setColor(CONFIG.colors.warning)], ephemeral: true });
                    setTimeout(async () => {
                        await interaction.channel.delete();
                        await Logger.discord('Ticket Fermé', `Le ticket a été fermé par **${interaction.user.tag}**.`, CONFIG.colors.warning, [], interaction.user.displayAvatarURL());
                    }, 5000);
                }
            }
        });
    ...
    async start() {
        try {
            ...
            Logger.success('Connexion à Discord établie avec succès.');
        } catch (error) {
